...
class CharactersView(AbstractGameView):
    ...
    def get_template_vars(self, previous_form_data=None):
        ...
        show_others_belongings = self.datamanager.is_master() or self.datamanager.has_permission(
            permission="view_others_belongings")
    ...
    def transfer_money(self, recipient_name, amount, sender_name=None, reason=None,
        ...
        self.datamanager.transfer_money_between_characters(from_name=sender_name,
                                                           ...
                                                           amount=amount,
                                                           # amount can only be positive here, thx to form validation
                                                           reason=reason)
...
@register_view(access=UserAccess.authenticated, title=ugettext_lazy("Auction Items"),
               title_for_master=ugettext_lazy("All Items"))  # fixme ? always available ?
def view_sales(request, template_name='auction/view_sales.html'):
    ...
    if user.is_master:
        ...
        if "buy" in params and "username" in params and "object" in params:
            with action_failure_handler(request, _("Object successfully transferred to '%s'.") % params[
                "username"].capitalize()):
            ...
            with action_failure_handler(request,
                                        _("Sale successfully canceled for '%s'.") % params["username"].capitalize()):
...
@register_view(access=UserAccess.authenticated, title=ugettext_lazy("Auction Slideshow"),
               title_for_master=ugettext_lazy("Items Slideshow"))
...
def ajax_chat(request):
    if request.method == "POST":
        ...
        for msg in new_messages:
            if not previous_msg_timestamp or (
        ...
        chatting_users = sorted(
            request.datamanager.build_visible_character_names(request.datamanager.get_chatting_users()))
