...
def trigger_akarith_attack(self, username, city_name, message):
    ...
    local_finish_time = utilities.utc_to_local(
        self.compute_effective_remote_datetime(self.get_global_parameter("akarith_attack_delays")))
...
def trigger_teldorian_teleportation(self, username, city_name, message):
    ...
    local_finish_time = utilities.utc_to_local(
        self.compute_effective_remote_datetime(self.get_global_parameter("teldorian_teleportation_delays")))
    local_finish_time_str = local_finish_time.strftime("%H:%M:%S")
...
def trigger_masslavian_mercenary_intervention(self, username, city_name, message):
    ...
    local_finish_time = utilities.utc_to_local(
        self.compute_effective_remote_datetime(self.get_global_parameter("mercenary_intervention_delays")))
    local_finish_time_str = local_finish_time.strftime("%H:%M:%S")
