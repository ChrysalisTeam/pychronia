...
def with_enforced_action_middlewares(action_name):
    ...
    def _execute_with_middlewares(method, self, *args, **kwargs):
        assert not getattr(method, "_is_under_transaction_watcher", None) or getattr(method,
                                                                                     "_is_under_readonly_method",
                                                                                     None)  # session management must be on TOP of decorators' stack
...
class DummyTestAbility(AbstractAbility):
    ...
    def reset_test_settings(self, action_name, middleware_class, new_settings):
        ...
        self.settings["middlewares"].setdefault(action_name, PersistentMapping())
        middleware_settings = self.settings["middlewares"][action_name].setdefault(middleware_class.__name__,
                                                                                   PersistentMapping())
    ...
    @with_enforced_action_middlewares(
        "middleware_wrapped_test_action")  # SHARED MIDDELWARE CONFIG, possible in tests only
