...
def GAME_INITIAL_FIXTURE_SCRIPT(dm):
    ...
    msg_id1 = dm.post_message(sender_email=email_guy1, recipient_email=email_guy2, subject="message1", body="hello")
    msg1 = dm.get_dispatched_message_by_id(msg_id1)
    msg_id2 = dm.post_message(sender_email=email_guy2, recipient_email=email_guy1, subject="RE:%s" % msg1["subject"], body="hello world", parent_id=msg_id1)
    msg2 = dm.get_dispatched_message_by_id(msg_id2)
    msg_id3 = dm.post_message(sender_email=email_guy1, recipient_email=email_guy2, subject="Bis:%s" % msg2["subject"], body="hello hello", parent_id=msg_id2)

    msg_id4 = dm.post_message(sender_email=email_guy1, recipient_email=email_master, subject="Ask master", body="for something")
    msg4 = dm.get_dispatched_message_by_id(msg_id4)
    msg_id5 = dm.post_message(sender_email=email_master, recipient_email=email_guy1, subject="RE:%s" % msg4["subject"], body="answer something", parent_id=msg_id4)
    msg5 = dm.get_dispatched_message_by_id(msg_id5)
    msg_id6 = dm.post_message(sender_email=email_guy1, recipient_email=email_master, subject="Bis:%s" % msg5["subject"], body="ask for something", parent_id=msg_id5)
