...
window.onload=function()
{
	...
	if (Browser.Version() <= 7) {
        return; // slideshow has big problems with odl IEs, as dynamic class/style changes are buggy
    }
...
// Create counter and previous/next list
	...
	function showslide(o)
	{
		...
		for(var i=1;i<slides.length;i++)
		{
			slides[i].className=hideClass; //DOESNT WORK IN ALL IEs
			/*slides[i].style.top=9999;
            slides[i].style.left=9999;
            slides[i].style.position="absolute";*/
           
		}
		if(slides[o])
		{	
			slides[o].className=slideClass; //DOESNT WORK IN ALL IEs
			/*slides[o].style.display="none";
			slides[o].style.display="block";
			slides[o].style.top="0";
			slides[o].style.left="0";
			slides[o].style.position="relative";*/
		}
		 
		/* IE hack, not working...
		alert("====");
	    var refresher=document.getElementById("refresher");
	    if (refresher.className==""){
	       refresher.className="hide";  
	    }
	    else refresher.className="";
        alert("-----");
        */
