...
(function( $ ) {
	...
	var vars = {
		...
		stepCache : [],
		latency : 0,
		burnoutMod : 1000, // modulo for early burnout. higher number allows particles to last longer.
		renderQuality : 50, // on a scale of 0-100
		renderQualityAcc : 0,
		renderQualityCt : 0,
		renderQualityAvg : 50,
		startTime : null
...
*/
	...
	Fireworks.prototype.explodeRing = function(pos, mag, op) {
		...
		for ( var i = 0; i < numP; ++i ) {
			...
			var myVel = vel.copy().rotateX(rX).rotateZ(rZ).multiplyEq( 6 + Math.random() / 5 );
			this.getParticle({
				pos: myPos.copy(),
				vel: myVel,
				grav: 0.3,
	...
	Fireworks.prototype.explodeCore = function(pos, mag) {
		...
		for ( var i = 0; i < numP; ++i ) {
			...
			var myVel = vel.copy().multiplyEq( Math.random() / 2 + 3.25);
			this.getParticle({
				pos: pos.copy(),
				vel: myVel,
				grav: 0.2,
